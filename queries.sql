...
SELECT t.name_task, t.file_task, date_format(t.done_at, '%d.%m.%Y') AS done_at, p.id, p.name_project, t.deadline
  FROM Task t JOIN Project p ON t.project_id = p.id where t.user_id = 1
   and project_id = 3;
